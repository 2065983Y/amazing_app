...
def add_maze(name, rows, cols, cells, attempts, solved_by="John"):
    maze = Maze.objects.get_or_create(name=name, rows=rows, cols=cols, cells=cells, solved_by=solved_by,
                                      attempts=attempts)
    return maze
...
def add_profile(username, email, password, created, solved):
    ...
    profile = UserProfile.objects.get_or_create(user=user, email=email, password=password, mazes_created=created,
                                                mazes_solved=solved)
    return profile
